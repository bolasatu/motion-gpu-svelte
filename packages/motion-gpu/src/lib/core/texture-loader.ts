...
function toBodyFingerprint(body: BodyInit | null | undefined): string | null {
  ...
  if (typeof FormData !== "undefined" && body instanceof FormData) {
    const entries = Array.from(body.entries()).map(
...
function normalizeTextureLoadOptions(
  options: TextureLoadOptions,
): NormalizedTextureLoadOptions {
  ...
  const normalized: NormalizedTextureLoadOptions = {
    ...
    decode: {
      colorSpaceConversion:
        options.decode?.colorSpaceConversion ??
        (colorSpace === "linear" ? "none" : "default"),
  ...
  const entry: TextureResourceCacheEntry = {
    ...
    blobPromise: fetch(url, requestInit)
      .then(async (response) => {
        if (!response.ok) {
          throw new Error(
            `Texture request failed (${response.status}) for ${url}`,
          );
        }
...
export async function loadTexturesFromUrls(
  ...
  const settled = await Promise.allSettled(
    urls.map((url) => loadTextureFromUrl(url, options)),
  );
  const loaded: LoadedTexture[] = [];
