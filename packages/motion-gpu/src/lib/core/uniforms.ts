 ...
 */
type UniformTypedInput = Extract<
  UniformValue,
  { type: UniformType; value: unknown }
>;
...
export function inferUniformType(value: UniformValue): UniformType {
  ...
  throw new Error(
    "Uniform value must resolve to f32, vec2f, vec3f, vec4f or mat4x4f",
  );
}
 ...
 */
export function assertUniformValueForType(
  type: UniformType,
  value: UniformValue,
): void {
