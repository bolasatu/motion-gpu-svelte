...
function buildTextureBindings(textureKeys: string[]): string {
  ...
  for (let index = 0; index < textureKeys.length; index += 1) {
    ...
    declarations.push(
      `@group(0) @binding(${binding}) var ${key}Sampler: sampler;`,
    );
    declarations.push(
      `@group(0) @binding(${binding + 1}) var ${key}: texture_2d<f32>;`,
    );
  }
...
export function buildShaderSourceWithMap(
  ...
  const fragmentStartLine = code
    .slice(0, fragmentStartIndex)
    .split("\n").length;
...
export function formatShaderSourceLocation(
  location: MaterialSourceLocation | null,
): string | null {
  if (!location) {
    return null;
  }
