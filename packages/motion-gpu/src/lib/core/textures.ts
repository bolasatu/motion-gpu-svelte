...
export function normalizeTextureDefinition(
  ...
  const normalized: NormalizedTextureDefinition = {
    ...
    format:
      definition?.colorSpace === "linear" ? "rgba8unorm" : "rgba8unorm-srgb",
    ...
    anisotropy: Math.max(
      1,
      Math.min(16, Math.floor(definition?.anisotropy ?? 1)),
    ),
...
export function resolveTextureSize(data: TextureData): {
  ...
  const width =
    data.width ?? source.naturalWidth ?? source.videoWidth ?? source.width ?? 0;
  const height =
    data.height ??
 ...
 */
export function isVideoTextureSource(
  source: TextureData["source"],
