...
export function planRenderGraph(
  ...
  const availableSlots = new Set<RenderPassInputSlot | RenderPassOutputSlot>([
    "source",
  ]);
  ...
  for (const pass of passes ?? []) {
    ...
    const output: RenderPassOutputSlot =
      pass.output ?? (needsSwap ? "target" : "source");
    ...
    if (!availableSlots.has(input)) {
      throw new Error(
        `Render pass #${enabledIndex} reads "${input}" before it is written.`,
      );
    }
