...
const INCLUDE_DIRECTIVE_PATTERN =
...
function normalizeTypedDefine(
  ...
  if (define.type === "bool") {
    if (typeof value !== "boolean") {
      throw new Error(
        `Invalid define value for "${name}". bool define requires boolean value.`,
      );
    }
  ...
  if (typeof value !== "number" || !Number.isFinite(value)) {
    throw new Error(
      `Invalid define value for "${name}". Numeric define must be finite.`,
    );
  }

  if (
    (define.type === "i32" || define.type === "u32") &&
    !Number.isInteger(value)
  ) {
    throw new Error(
      `Invalid define value for "${name}". ${define.type} define requires integer.`,
    );
  }

  if (define.type === "u32" && value < 0) {
    throw new Error(
      `Invalid define value for "${name}". u32 define must be >= 0.`,
    );
  }
 ...
 */
export function normalizeDefines(
  ...
  for (const [name, value] of Object.entries(defines ?? {})) {
    ...
    if (typeof value === "number") {
      if (!Number.isFinite(value)) {
        throw new Error(
          `Invalid define value for "${name}". Define numbers must be finite.`,
        );
      }
      resolved[name] = value;
      continue;
...
export function normalizeIncludes(
  ...
  for (const [name, source] of Object.entries(includes ?? {})) {
    ...
    if (typeof source !== "string" || source.trim().length === 0) {
      throw new Error(
        `Invalid include "${name}". Include source must be a non-empty WGSL string.`,
      );
    }
...
export function toDefineLine(key: string, value: MaterialDefineValue): string {
  ...
  if (value.type === "f32") {
    ...
    const valueLiteral = Number.isInteger(numberValue)
      ? `${numberValue}.0`
      : `${numberValue}`;
    return `const ${key}: f32 = ${valueLiteral};`;
  }
...
function expandChunk(
  ...
  for (let index = 0; index < sourceLines.length; index += 1) {
    ...
    if (!includeSource) {
      throw new Error(
        `Unknown include "${includeKey}" referenced in fragment shader.`,
      );
    }
...
export function preprocessMaterialFragment(input: {
  ...
  const defineEntries = Object.entries(normalizedDefines).sort(([a], [b]) =>
    a.localeCompare(b),
  );
  const lines: string[] = [];
