...
function isMaterialSourceMetadata(
  value: unknown,
): value is MaterialSourceMetadata {
...
function isMaterialSourceLocation(
  value: unknown,
): value is MaterialSourceLocation | null {
  if (value === null) {
    return true;
  }
...
function isShaderCompilationDiagnostic(
  value: unknown,
): value is ShaderCompilationDiagnostic {
  if (value === null || typeof value !== "object") {
    return false;
  }
  ...
  if (
    record.lineLength !== undefined &&
    typeof record.lineLength !== "number"
  ) {
    return false;
  }
...
export function getShaderCompilationDiagnostics(
  ...
  if (
    record.includeSources === null ||
    typeof record.includeSources !== "object"
  ) {
    return null;
  }
  const includeSources = record.includeSources as Record<string, unknown>;
  if (
    Object.values(includeSources).some((value) => typeof value !== "string")
  ) {
    return null;
  }
  if (
    record.materialSource !== null &&
    !isMaterialSourceMetadata(record.materialSource)
  ) {
    return null;
  }
  ...
  return {
    ...
    materialSource: (record.materialSource ??
      null) as MaterialSourceMetadata | null,
  };
}
