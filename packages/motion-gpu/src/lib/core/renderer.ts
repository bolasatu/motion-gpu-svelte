...
function uploadTexture(
  ...
  binding: Pick<
    ...
    "flipY" | "premultipliedAlpha" | "generateMipmaps"
  >,
...
export async function createRenderer(
  ...
  const context = options.canvas.getContext(
    "webgpu",
  ...
  try {
    const convertLinearToSrgb = shouldConvertLinearToSrgb(
    ...
    const textureBindings = options.textureKeys.map(
      (key, index): RuntimeTextureBinding => {
    ...
    const uniformScratch = new Float32Array(
      options.uniformLayout.byteLength / 4,
    );
    const uniformPrevious = new Float32Array(
      options.uniformLayout.byteLength / 4,
    );
    ...
    const updateTextureBinding = (
      ...
      if (!requiresReallocation) {
        const shouldUpload =
          update === "perFrame" ||
          (update === "onInvalidate" &&
            (renderMode !== "always" || tokenChanged));
    ...
    const syncRenderTargets = (
      ...
      if (nextSignature !== renderTargetSignature) {
        const activeKeys = new Set(
          resolvedDefinitions.map((definition) => definition.key),
        );
    ...
    const render: Renderer["render"] = ({
      ...
      if (!hasUniformSnapshot) {
        ...
        const dirtyRanges = findDirtyFloatRanges(
          uniformPrevious,
          uniformScratch,
        );
      ...
      for (const binding of textureBindings) {
        const nextTexture =
          textures[binding.key] ??
          normalizedTextureDefinitions[binding.key]?.source ??
          null;
      ...
      if (slots) {
        ...
        if (graphPlan.finalOutput !== "canvas") {
          const finalSurface =
            graphPlan.finalOutput === "source" ? slots.source : slots.target;
          blitToCanvas(
            commandEncoder,
            finalSurface.view,
            slots.canvas.view,
