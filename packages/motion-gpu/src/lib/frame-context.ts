 ...
 */
export type FrameStageCallback = (
  state: FrameState,
  runTasks: () => void,
) => void;
 ...
 */
const DEFAULT_STAGE_CALLBACK: FrameStageCallback = (_state, runTasks) =>
  runTasks();
...
function sortByDependencies<T extends { key: FrameKey; order: number }>(
  ...
  if (ordered.length !== items.length) {
    const missing = items.filter(
      (item) => !ordered.some((entry) => entry.key === item.key),
    );
...
export function createFrameRegistry(options?: {
  ...
  let profilingEnabled =
    options?.profilingEnabled ?? options?.diagnosticsEnabled ?? false;
  ...
  const buildProfilingSnapshot = (): FrameProfilingSnapshot | null => {
    ...
    for (const frame of profilingHistory) {
      ...
      for (const [stageKey, stageTiming] of Object.entries(frame.stages)) {
        ...
        for (const [taskKey, taskDuration] of Object.entries(
          stageTiming.tasks,
        )) {
    ...
    for (const [stageKey, stageBucket] of stageBuckets) {
      ...
      for (const [taskKey, taskDurations] of stageBucket.taskDurations) {
        const lastTaskDuration =
          lastRunTimings?.stages[stageKey]?.tasks[taskKey] ?? 0;
        taskSnapshot[taskKey] = buildTimingStats(
          taskDurations,
          lastTaskDuration,
        );
      }
  ...
  const ensureStage = (
    ...
    if (existing) {
      if (stageOptions?.before !== undefined) {
        existing.before = new Set(
          stageOptions.before.map((entry) => toStageKey(entry)),
        );
        markScheduleDirty();
      }
      if (stageOptions?.after !== undefined) {
        existing.after = new Set(
          stageOptions.after.map((entry) => toStageKey(entry)),
        );
        markScheduleDirty();
      }
      if (
        stageOptions &&
        Object.prototype.hasOwnProperty.call(stageOptions, "callback")
      ) {
  ...
  return {
    register(keyOrCallback, callbackOrOptions, maybeOptions) {
      ...
      const callback =
        typeof keyOrCallback === "function"
          ? keyOrCallback
          : (callbackOrOptions as FrameCallback);
      const taskOptions =
        typeof keyOrCallback === "function"
      ...
      for (const stage of sortedStages) {
        ...
        stage.callback(frameState, () => {
          for (const task of taskList) {
            ...
            if (profilingEnabled) {
              taskTimings[keyToString(task.task.key)] =
    ...
    createStage(key, options) {
      const stageOptions: Parameters<typeof ensureStage>[1] | undefined =
        options
          ? {
