...
describe("frame registry", () => {
  ...
  it("clamps delta passed to callbacks and supports runtime updates", () => {
    ...
    expect(callback).toHaveBeenLastCalledWith(
      expect.objectContaining({ delta: 0.01 }),
    );
  });
  ...
  it("provides schedule snapshot and optional frame timings diagnostics", () => {
    ...
    expect(
      schedule.stages.find((stage) => stage.key === "early")?.tasks,
    ).toContain("early-task");
    ...
    expect(timings?.stages.early?.tasks["early-task"]).toBeGreaterThanOrEqual(
      0,
    );
