...
<button
  type="button"
  class={cn(
    "group transition-scale relative flex size-7 bg-card border border-border rounded-md cursor-pointer items-center justify-center text-foreground duration-150 ease-out active:scale-[0.95]",
    className,
  )}
  onclick={(event) => {
