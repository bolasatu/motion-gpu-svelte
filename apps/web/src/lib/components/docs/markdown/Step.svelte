...
<div class={cn("relative pb-10 pl-8", className)}>
  {#if title}
    <div class="mb-4 flex h-8 items-center">
      <h3
        class="relative z-10 text-base leading-none font-medium size-8 items-center justify-center rounded-full border border-border bg-card text-foreground [counter-increment:step] before:content-[counter(step)]"
      >
        {title}
      </h3>
    </div>
  {:else}
    ...
    ></span>
  {/if}
  <div class="text-base leading-relaxed text-foreground/70 text-pretty">
    {@render children?.()}
  </div>
</div>
